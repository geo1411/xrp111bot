# xrp111Bot — Telegram Crypto Signals (Python)

**Features**
- `/set <SYMBOL> <INTERVAL>` (e.g. `/set BTCUSDT 1h`)
- `/signal` one symbol
- `/watchlist [SYMBOLS...]` set/view list
- `/watchadd SYMBOLS...` add coins
- `/watchrm SYMBOLS...` remove coins
- `/watch` summaries for all
...
printf "BOT_TOKEN=YOUR_TELEGRAM_TOKEN\n" > .env
python crypto_signal_bot.py

source venv/bin/activate
nohup env PYTHONUNBUFFERED=1 ./venv/bin/python -u crypto_signal_bot.py >> ~/xrp111bot.log 2>&1 & disown
...
[Service]
User=YOUR_USER
WorkingDirectory=/home/YOUR_USER/xrp111bot
EnvironmentFile=/home/YOUR_USER/xrp111bot/.env
ExecStart=/home/YOUR_USER/xrp111bot/venv/bin/python /home/YOUR_USER/xrp111bot/crypto_signal_bot.py
Restart=always
RestartSec=5
StandardOutput=append:/home/YOUR_USER/xrp111bot.log
StandardError=append:/home/YOUR_USER/xrp111bot.log
[Install]
WantedBy=multi-user.target

sudo systemctl daemon-reload
sudo systemctl enable xrp111bot
sudo systemctl start xrp111bot
sudo systemctl status xrp111bot --no-pager
...
Create the repo locally and push. Replace `USERNAME` with your GitHub username.


mkdir -p ~/bots/xrp111bot
cd ~/bots/xrp111bot

cat > crypto_signal_bot.py << 'PY'
...
cat > README.md << 'MD'
# xrp111Bot — Telegram Crypto Signals
